...
class SMTPConnection extends EventEmitter {
    ...
    _supportedExtensions: any[];

    _maxAllowedSize: number;

    _responseActions: any[];
    _recipientQueue: any[];

    _greetingTimeout: NodeJS.Timeout;
    ...
    constructor(options) {
         ...
         * @private
         */
        this._greetingTimeout = undefined;
     ...
     */
    _processResponse(b?: boolean) {
        if (!this._responseQueue.length) {
            return false;
        }

        const str = (this.lastServerResponse = (
    ...
    _actionAUTH_CRAM_MD5(str, callback) {
        ...
        // Decode from base64
        const base64decoded = Buffer.from(challengeString, 'base64').toString(
                'ascii'
            ),
        ...
        const prepended = this._auth.credentials.user + ' ' + hex_hmac;
    ...
    _handleXOauth2Token(isRetry, callback) {
        this._auth.oauth2.getToken(isRetry, (err, accessToken) => {
            ...
            this._sendCommand(
                'AUTH XOAUTH2 ' +
                    this._auth.oauth2.buildXOAuth2Token(accessToken)
            );
        });
    }
